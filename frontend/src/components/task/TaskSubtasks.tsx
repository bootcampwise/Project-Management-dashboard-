    ...
    return (
        <div className="mb-8">
            <div className="flex items-center gap-2 mb-3">
                <h3 className="text-sm font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">Subtasks</h3>
                <span className="text-gray-400 dark:text-gray-500 text-xs font-medium">· {completedCount}/{totalCount}</span>
            </div>
            ...
            {totalCount > 0 && (
                <div className="flex items-center gap-3 mb-4">
                    ...
                    <span className={`text-xs font-medium min-w-[40px] ${subtaskProgress === 100 ? 'text-green-600 dark:text-green-400' : 'text-gray-500 dark:text-gray-400'}`}>
                        {subtaskProgress}%
                    </span>
                </div>
            )}
            ...
            <div className="space-y-1">
                {subtasks?.map((subtask) => (
                    <div key={subtask.id} className="flex items-center justify-between group py-2.5 hover:bg-gray-50 dark:hover:bg-gray-800 px-3 -mx-3 rounded-lg transition-colors">
                        <div className="flex items-center gap-3">
                            ...
                            >
                                <CheckCircle2
                                    size={18}
                                    className={`cursor-pointer transition-colors ${subtask.completed ? 'text-green-500' : 'text-gray-300 dark:text-gray-600 hover:text-gray-400 dark:hover:text-gray-500'}`}
                                />
                            </button>
                            <span className={`text-sm ${subtask.completed ? 'text-gray-400 dark:text-gray-500 line-through' : 'text-gray-700 dark:text-gray-200 font-medium'}`}>
                                {subtask.title}
                            </span>
                        </div>
                        ...
                        <div className="flex items-center gap-2">
                            {/* Creator avatar */}
                            {subtask.createdBy && (
                                <div className="flex items-center gap-1" title={`Created by ${subtask.createdBy.name}`}>
                                    {subtask.createdBy.avatar ? (
                                        <img src={subtask.createdBy.avatar} alt={subtask.createdBy.name} className="w-5 h-5 rounded-full opacity-60" />
                                    ) : (
                                                ...
                                                filteredTeamMembers.map((member) => {
                                                    ...
                                                    return (
                                                        <button
                                                            key={member.id}
                                                            onClick={() => {
                                                                onAssign(subtask.id, String(member.id), isAssigned ? 'remove' : 'add');
                                                            }}
                                                            className={`w-full flex items-center gap-2 px-3 py-2 text-sm hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors ${isAssigned ? 'bg-blue-50 dark:bg-blue-900/30' : ''}`}
                                                        >
                                                            <div className={`w-4 h-4 rounded border flex items-center justify-center flex-shrink-0 ${isAssigned ? 'bg-blue-500 border-blue-500' : 'border-gray-300 dark:border-gray-600'}`}>
                                                                {isAssigned && <CheckCircle2 size={12} className="text-white" />}
                                                            </div>
                                                            {member.avatar ? (
                                                            ...
                                                            <span className={`truncate ${isAssigned ? 'text-blue-700 dark:text-blue-400 font-medium' : 'text-gray-700 dark:text-gray-200'}`}>{member.name}</span>
                                                        </button>
                                                    );
                                                })
                                            ) : (
