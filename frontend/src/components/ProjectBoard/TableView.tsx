      ...
      {groups.map((group) => (
        <div key={group.id} className="mb-8">
          ...
          <div className="flex items-center gap-2 mb-2 px-2 group/header">
            ...
            <h3 className="text-[15px] font-semibold text-gray-800 dark:text-white">{group.title}</h3>
            <span className="text-gray-400 dark:text-gray-500 font-normal ml-1">{group.count}</span>
          ...
          {!group.isCollapsed && (
            <div>
              {/* Column Headers */}
              <div className={`grid gap-0 border-y border-gray-100 dark:border-gray-700 text-xs text-gray-500 dark:text-gray-400 font-normal bg-white dark:bg-gray-800 sticky top-0 z-10`}
                style={{ gridTemplateColumns: `1fr ${visibleFields.assignee ? '180px ' : ''}${visibleFields.dueDate ? '100px ' : ''}${visibleFields.label ? '180px ' : ''}40px` }}
              >
                <div className="px-4 py-2 border-r border-gray-100 dark:border-gray-700">Name</div>
                {visibleFields.assignee && <div className="px-4 py-2 border-r border-gray-100 dark:border-gray-700">Assignee</div>}
                {visibleFields.dueDate && <div className="px-4 py-2 border-r border-gray-100 dark:border-gray-700">Due date</div>}
                {visibleFields.label && <div className="px-4 py-2 border-r border-gray-100 dark:border-gray-700">Label</div>}
                  ...
                  group.tasks.map((task) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3 px-4 h-full border-r border-gray-100 dark:border-gray-700 overflow-hidden">
                        ...
                        <span className="text-gray-800 dark:text-gray-200 truncate font-normal">{task.name}</span>
                        ...
                        <div className="flex items-center gap-2 h-full px-4 border-r border-gray-100 dark:border-gray-700">
                          {!task.assignee || task.assignee.name === 'Unassigned' || !task.assignee.name ? (
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <span className="truncate text-gray-700 dark:text-gray-300">{task.assignee.name}</span>
                            </div>
                          ) : (
                            <div className="flex items-center gap-2">
                      ...
                      {visibleFields.label && (
                        <div className="flex items-center gap-1.5 h-full px-4 border-r border-gray-100 dark:border-gray-700 overflow-hidden">
                          {task.labels.map((label, idx) => {
                            const bgClass = label.bg && label.bg.length > 2 ? label.bg : 'bg-gray-100 dark:bg-gray-700';
                            const textClass = label.color && label.color.length > 2 ? label.color : 'text-gray-600 dark:text-gray-300';
                            return (
              ...
              >
                ...
                <span className="group-hover/add:text-gray-800 dark:group-hover/add:text-gray-200 transition-colors">Add task...</span>
              </button>
            </div>
          )}
        </div>
